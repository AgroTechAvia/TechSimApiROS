...
class EduSimDriver(Node):
    def __init__(self):
        ...
        PORT = 41451
        self.sim_client = MultirotorClient(ip = HOST)
        self.is_connected_to_server = self.connect_to_server()

        self.sim_client.reset()
        self.sim_client.enableApiControl(True)
        print("API Control enabled: %s" % self.sim_client.isApiControlEnabled())
        self.sim_client.setMode(AirMultirotorMode.STABILIZE)
        self.sim_client.setMode(AirMultirotorMode.COPTER)
    ...
    def cmd_callback(self, data):
        ...
        if(data.angular.x > 0):
            self.sim_client.armDisarm(True)
        else:
            self.sim_client.armDisarm(False)
        if(data.angular.y > 0):
            self.sim_client.setMode(AirMultirotorMode.PLANE)
        else:
            self.sim_client.setMode(AirMultirotorMode.COPTER)
        #self.get_logger().info(str(roll) + str(pitch) + str(yaw) + str(trust))
        self.sim_client.moveByRollPitchYawThrottleAsync(roll, pitch, yaw, trust, 0.05)
